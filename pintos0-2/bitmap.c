...
functionality : expand the given bitmap to the size

*/
struct bitmap* bitmap_expand(struct bitmap *bitmap, int size){
  int default_size = bitmap->bit_cnt;
  elem_type * default_bits = bitmap->bits;
  struct bitmap *creation_bitmap =  bitmap_create(size+default_size);

  for(int i =0; i<default_size; i++){
    if(bitmap_test(bitmap, i)){
      bitmap_mark(creation_bitmap, i);
    }
  }
  bitmap_destroy(bitmap);
  
  return creation_bitmap;
}
